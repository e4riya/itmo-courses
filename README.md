...
1. Запустить БД: `docker compose up -d postgres mongodb`.
2. Запустить приложение: `./gradlew bootRun`.
3. Открыть `http://localhost:8080`.
4. Показать каталог курсов и цены в рублях.
5. Открыть детальную страницу курса.
6. Создать оплату `PAID` и показать, что enrollment создался триггером.
7. Завершить урок и показать пересчет прогресса.
8. Открыть кабинет студента и показать сертификат.
9. Добавить отзыв и показать, что он хранится в MongoDB.
10. Открыть `/analytics` и показать native SQL-отчеты.
11. Через `psql` показать таблицы, триггеры и пример `EXPLAIN`.

## Материалы для защиты

- Презентация: `docs/online-courses-presentation.pptx`
- Разбор проекта по коду и БД: `PROJECT_GUIDE.md`
- Текст для выступления по слайдам: `PRESENTATION_SPEECH.md`
